...
#include "autons.hpp"
#include "config.hpp"
#include "libs/include/EZ-Template/tracking_wheel.hpp"
#include "libs/include/maelstrom/logging.hpp"

inline void maelstromtask(){
    maelstrom::logging::init(true,true, CONFIG::getLeftDrv(), CONFIG::getRightDrv(), true);
   // maelstrom::logging::robot_faults_log();
    //maelstrom::logging::robot_coords_log();
    while(true){
       maelstrom::logging::set_robot_coords(EZchassis.odom_x_get(),EZchassis.odom_y_get(), EZchassis.odom_theta_get());
    }
}
...
void initialize() {
  ...
  //Maelstrom Init
  pros::Task maelstromTask(maelstromtask);
  ...
  EZchassis.opcontrol_curve_default_set(0.0, 0.0);  // Defaults for curve. If using tank, only the first parameter is used. (Comment this line out if you have an SD card!)
  //Tracking Wheels (Uncomment if in use)
